...
/* static */ int32_t R_BSP_Aio::sync_trans(rbsp_serial_ctl_t * p_ctl, void * const p_data, uint32_t data_size) {
    ...
    if (p_sem_wait != NULL) {
        ...
        if (aio_trans(p_ctl, p_data, data_size, &data_conf) == ESUCCESS) {
            // 【修正】mbed-os 6以降、rtos::Semaphoreの公開APIから
            // wait(uint32_t)が削除され、代わりにvoid acquire()（無条件で
            // ブロックし、成功するまで戻らない）に変更されている。
            // 旧コードのosWaitForever(無期限待ち)は元々acquire()と
            // 意味的に等価なので、単純に置き換えるだけでよい。
            p_sem_wait->acquire();
        }
        delete p_sem_wait;
    }
...
/* static */ int32_t R_BSP_Aio::aio_trans(rbsp_serial_ctl_t * const p_ctl,
    ...
    if ((p_data_conf == NULL) || (p_data == NULL)) {
        ...
        // 【修正】mbed-os 6以降、rtos::Semaphoreのwait(uint32_t)が廃止され
        // void acquire()（無条件で成功するまでブロック、エラー戻り値なし）
        // に変更された。旧コードはosWaitForever(無期限待ち)を使っていたため
        // 元々「失敗する経路」は実質的に存在しなかった。そのため
        // acquire()に置き換えるだけで動作は変わらない
        // （EIO_RBSPのエラー分岐は到達しなくなるため削除した）。
